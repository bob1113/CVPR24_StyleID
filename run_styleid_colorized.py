...
def load_img_with_saturation_boost(path, saturation_factor=1.3):
    """
    加載圖片並增強色彩飽和度。
    
    Args:
        path (str): 圖片的路徑。
        saturation_factor (float): 色彩飽和度的增強倍數（默認 1.5）。
                                  1.0 表示不變，>1 增強，<1 減弱。
    Returns:
        torch.Tensor: 經處理的圖像張量。
    """
    # 打開圖片並轉換為 RGB 格式
    image = Image.open(path).convert("RGB")
    x, y = image.size
    ...
    image = enhancer.enhance(saturation_factor)  # 增強飽和度
...
def load_img_with_red_boost(path, red_boost=0.2):
    image = Image.open(path).convert("RGB")
    x, y = image.size
    print(f"Loaded input image of size ({x}, {y}) from {path}")
    h = w = 512
    ...
    #green_boost = 0.01
    #blue_boost = 0.01
    image[:, :, 0] = np.clip(image[:, :, 0] + red_boost, 0.0, 1.0)  # 只增強第 0 通道（紅色）
    #image[:, :, 1] = np.clip(image[:, :, 0] + green_boost, 0.0, 1.0)  # 只增強第 0 通道（紅色）
    #image[:, :, 2] = np.clip(image[:, :, 0] + blue_boost, 0.0, 1.0)  # 只增強第 0 通道（紅色）
...
#     sty_mean = sty_feat.mean(dim=[0, 2, 3],keepdim=True)
#     sty_std = sty_feat.std(dim=[0, 2, 3],keepdim=True)
#     output = ((cnt_feat-cnt_mean)/cnt_std)*sty_std*10 + sty_mean*10
#     return output
#======================================
